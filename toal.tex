...
\begin{defn}
  Una cubierta $(\XX,p)$ de un espacio topológico $X$ se dice \emph{regular} si $p_*\gf(\XX,\xx)\lhd \gf(X,x)$ y se dice \emph{universal} si $\XX$ es simplemente conexo.  
\end{defn}

\begin{prop}
  Sea $X$ un espacio topológico. Si $(\XX,p)$ es una cubierta regular de $X$, entonces $\DD(p)\cong \gf(X,x)/p_*\gf(X,x)$. Si $(\XX,p)$ es una cubierta universal de $X$, $\DD(p)\cong \gf(X,x)$.
\end{prop}
...
\begin{thm}
  Un espacio topológico $X$ admite una cubierta universal si y sólo si $X$ es conexo por caminos y localmente semisimplemente conexo.
\end{thm}

\begin{proof}
  Se trata de construir la cubierta $(X,p)$ y ver que es cubierta universal. 

  \textbf{Construir la cubierta}. Fijado $x\in X$, definimos $\XX$ como las clases de caminos en $X$ con origen $x$ por la relación de homotopía con extremos fijos y la proyección
  \begin{align*}
    p :\XX&\longrightarrow X\\ 
    [\alpha] &\longmapsto \alpha(1) .
    \end{align*}

    \textbf{Dar la topología}. Si $\mathcal{U}$ es una familia de abiertos de $X$ conexos por caminos y tales que todo lazo en uno de ellos es nulhomótopo en $X$. Dado $U\in \mathcal{U}$, llamamos
    \begin{equation*}
...
\end{enumerate}
    ...
    \textbf{Conexo por caminos}. Dado $[\gamma]\in \XX$ basta considerar la función
    \begin{align*}
      I&\longrightarrow \XX\\ 
